...
if luci.model.uci.cursor():get_first("bypass","global","nf_server",'0') ~= '0' then
...
if luci.sys.call("uci -q get bypass.@global[0].run_mode | grep router >/dev/null")==0 then
...
s.template="bypass/check"

s=m:field(DummyValue,"baidu",translate("Baidu Connectivity"))
s.value=translate("No Check")
s.template="bypass/check"

if nixio.fs.access("/tmp/bypass/gfw.list") then
	s=m:field(DummyValue,"gfw_data",translate("GFW List Data"))
	s.rawhtml=true
	s.template="bypass/refresh"
	s.value=luci.sys.exec("cat /tmp/bypass/gfw.list | wc -l").." "..translate("Records")
end

if nixio.fs.access("/tmp/bypass/china.txt") then
	s=m:field(DummyValue,"ip_data",translate("China IPv4 Data"))
	s.rawhtml=true
	s.template="bypass/refresh"
	s.value=luci.sys.exec("cat /tmp/bypass/china.txt | wc -l").." "..translate("Records")
end

if nixio.fs.access("/tmp/bypass/china_v6.txt") then
	s=m:field(DummyValue,"ip6_data",translate("China IPv6 Data"))
	s.rawhtml=true
	s.template="bypass/refresh"
	s.value=luci.sys.exec("cat /tmp/bypass/china_v6.txt | wc -l").." "..translate("Records")
end

s=m:field(DummyValue,"check_port",translate("Check Server Port"))
s.template="bypass/checkport"
s.value=translate("No Check")
